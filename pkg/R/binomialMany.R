...
function (link = "logit")
{
    ...
    if (!is.character(linktemp)) {
        ...
        if (linktemp == "link") {
          warning("use of binomialMany(link=link) is deprecated\n",
                domain = NA)
            linktemp <- eval(link)
    ...
    if(linktemp=="varstab"){
        stats     <- make.vslink("binomial")
        linktemp  <- stats$name  
# however this hasn't an attribute, as there is no easy analogue
    ...
    dev.resids <- function(y, mu, wt) {
      ...
      for(i in 1:q)
        dr[,i] <- .Call("binomial_dev_resids",
           y[,i], mu[,i], wt, PACKAGE = "stats")
      return(dr)
     }
    aic <- function(y, n, mu, wt, dev) {
        ...
        m <- if (any(n > 1))
            n
        else matrix(rep(wt, times=q),nrow=nrow(y), ncol=q)
        aici <- NULL
        for(i in 1:q)  {
        aici[i] <-
        -2 * sum(ifelse(m[,i] > 0, (wt/m[,i]), 0) * dbinom(round(m[,i] *
            y[,i]), round(m[,i]), mu[,i], log = TRUE))     }   #
        return(aici)
    }
    initialize <- expression({
      ...
      #  if (is.null(attr(y, "structure"))) {
            ...
            weights <- n[,1] * weights    
		# this is a matrix --> errors possible
            mustart <- (n * y + 0.5)/(n + 1)
            
        } # else stop("for the binomialMany family, y must be a matrix of 0 and 1's\n",
    ...
    structure(list(family = "binomial", link = linktemp, 
	  ...
	  initialize = initialize, validmu = validmu, 
        valideta = stats$valideta), class =
        c("family.mvabund", "family"))
}
