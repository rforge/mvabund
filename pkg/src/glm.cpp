   ...
   *tol = 1.0;
   ...
   while ( (*tol>mmRef->tol) ) {
       ...
       for (i=0; i<nRows; i++) {
           ...
           if ((eij>-log(eps))|(eij<log(eps))) good[i]=FALSE;
...
//       gsl_vector_set_zero (&wj.vector);
       ...
       // As in glm2, solve WXb=Wz, David suggested not good 
       // So back to solving X'WXb=X'Wz
       gsl_blas_dgemv(CblasTrans,1.0,WX,z,0.0,Xwz);
       gsl_matrix_set_zero (XwX);
//     gsl_blas_dgemm(CblasTrans,CblasNoTrans,1.0,WX,WX,0.0,XwX); // if QR
       gsl_blas_dsyrk (CblasLower,CblasTrans,1.0,WX,0.0,XwX); 
       if (calcDet(XwX)<eps){ // test if singular or nan
          gsl_matrix_set_identity (XwX);
          gsl_blas_dsyrk (CblasLower,CblasTrans,1.0,WX,eps,XwX);     
//     gsl_blas_dgemm(CblasTrans,CblasNoTrans,1.0,WX,WX,eps,XwX); // if QR
       }   
       status=gsl_linalg_cholesky_decomp(XwX);
       if (status) {
...
int PoissonGlm::update(gsl_vector *bj, unsigned int id)
{
    ...
    for (i=0; i<nRows; i++) {
       ...
       if (Oref!=NULL) eij = eij+gsl_matrix_get(Oref, i, id);
       if ( (eij>-log(eps))|(eij<log(eps)) ) {
          isValid=FALSE;
       }
       mij = invLink(eij);
...
{   
    ...
    double a, fA, fAdash;
    double initphi=eps, phi_old, tol_grad, tol_old, tol=1.0;
    int status;
	...
	gsl_matrix_set_col(varBeta, j, &v0j.vector);
		...
		phi[j] = phi[j]*getDisper(j); // 1-step phi
	        if (tol<mmRef->tol) break; // Normal break;
