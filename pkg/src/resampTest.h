...
//    private: 
	   ...
	   unsigned int genRandist(double mui, double a) const  
                { return (unsigned int)Rf_rpois(mui); }
};
...
//    private: // logit link and property functions          
           ...
           double llfunc(double yi, double mui, double a) const
                { if (n==1) return 2*((yi==0)?(((1-mui)<TOL)?0:log(1-mui)):0+(yi==1)?((mui<TOL)?0:log(mui)):0);
                  else return 2*((yi>0)?yi*log(mui/n):0+(yi<n)?(n-yi)*log(1-mui/n):0+gsl_sf_lngamma(n+1)-gsl_sf_lngamma(yi+1)-gsl_sf_lngamma(n-yi+1)); }
           double devfunc(double yi, double mui, double a) const
                { if (n==1) return 2*((yi==0)?(((1-mui)<TOL)?0:-log(1-mui)):0+(yi==1)?((mui<TOL)?0:-log(mui)):0);
...
//    private:
	   ...
	   unsigned int genRandist(double mui, double a) const	   
                { if (a<TOL) return (unsigned int)Rf_rpois(mui);
                  else return (unsigned int)Rf_rnbinom(1/a, 1/(1+a*mui)); }
